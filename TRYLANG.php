...
<body>
  ...
  <script>
    ...
    function sendChat() {
      ...
      const match = msg.match(/(add|i want|order)?\s*(\d+)?\s*(.+)/i);
      if (match) {
        ...
        for (const category in menuItems) {
          menuItems[category].forEach((item, i) => {
            if (item.name.toLowerCase().includes(name)) {
              ...
              const qtyElem = document.getElementById(`${category}-qty-${i}`);
              if (qtyElem) qtyElem.innerText = item.quantity;
        ...
        if (!found) appendMessage("bot", "❌ Item not found.");
      } else {
        appendMessage("bot", "🤖 Please say something like: Add 2 Coke");
      }
